...
class HookSparqlEndpointComunica {
    constructor(dockerfileClient, resourceConstraints, configClient, contextClient, additionalBinds, clientPort, clientLogLevel, queryTimeout, maxMemory) {
    ...
    async start(context, options) {
        var _a;
        const additionalMaps = this.additionalBinds.map(x => path_1.default.join(context.experimentPaths.root, x));
